...
class MaterialManager implements IContextComponent, IConditionRegister {
     ...
     */
    public registerMaterial(matDocument: string): string { // TODO xml should not be parsed in this method
        const dom = XMLReader.parseXML(matDocument);
        const matTag = XMLReader.getSingleElement(dom, "material", true);
