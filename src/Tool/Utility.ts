 ...
 */
export function flatMap<T>(source: T[], map: (a: T) => T[]): T[] {
 ...
 */
export function remove<T>(array: T[], target: T): boolean {
  let index = -1;
  for (let i = 0; i < array.length; i++) {
    if (target === array[i]) {
      index = i;
 ...
 * @param array array
 */
export function checkOverlap<T>(array: T[]): boolean {
  const list = [];
  for (let i = 0; i < array.length; i++) {
    const item = array[i];
 ...
 */
export function getNodeListIndexByElementIndex(targetElement: Element, elementIndex: number): number {
 ...
 */
export function getAttributes(element: Element): { [key: string]: string } {
 ...
 * @param errorMessage
 */
export function assert(shouldTrue: boolean, errorMessage: string | (() => string)) {
  if (!shouldTrue) {
