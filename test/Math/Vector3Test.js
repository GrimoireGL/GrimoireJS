...
describe("Vector3 test",()=>{
  ...
  it("zero vector test",()=>{
    assert((new Vector3(1,0,0)).equalWith(new Vector3(1,0,0)));
  });
  it("Get the nunber of elements of a vector test",()=>{
    assert((new Vector3(1,1,2)).ElementCount === 2);
  });
  ...
  it("Set Y of a vector test",()=>{
    let vec = new Vector3(1,1,2);
    vec.Y = 1;
    assert(vec.equalWith(new Vector3(1,1,1)));
  });


  it("Dot vectors test",()=>{
    assert(Vector3.dot(new Vector3(1,1,2),new Vector3(1,2,3))===8);
  });
  it("Add vectors test",()=>{
    assert(Vector3.add(new Vector3(1,1,2),new Vector3(1,2,3)).equalWith(new Vector3(1,3,5)));
  });
  it("Subtract vectors test",()=>{
    assert(Vector3.subtract(new Vector3(1,2,3),new Vector3(1,1,2)).equalWith(new Vector3(1,1,1)));
  });
  it("Multiply a vector test",()=>{
    assert(Vector3.multiply(2,new Vector3(1,1,2)).equalWith(new Vector3(1,2,4)));
  });
  ...
  it("Normalize a vector test for x axis",()=>{
    assert(Vector3.normalize(new Vector3(1,2,0)).equalWith(new Vector3(1,1,0)));
  });

  it("Normalize a vector test for a leaning axis",()=>{
    assert(Vector3.normalize(new Vector3(1,3,4)).nearlyEqualWith(new Vector3(1,0.6,0.8)));
  });
  ...
  it("Create an angle from Vector3 test",()=>{
    assert(Math.abs(Vector3.angle(new Vector3(1,1,0),new Vector3(1,0,1)) - 1.5707963) <= 0.01);
  });
  it("A vector dot with another one test",()=>{
    assert((new Vector3(1,1,2)).dotWith(new Vector3(1,2,3))===8);
  });
  it("Add vectors test",()=>{
    assert((new Vector3(1,1,2)).addWith(new Vector3(1,2,3)).equalWith(new Vector3(1,3,5)));
  });
  it("Subtract vectors test",()=>{
    assert((new Vector3(1,1,2)).subtractWith(new Vector3(1,2,3)).equalWith(new Vector3(1,1,1)));
  });
  it("multiply vectors test",()=>{
    assert((new Vector3(1,1,2)).multiplyWith(2).equalWith(new Vector3(1,2,4)));
  });
  it("negate a vector test",()=>{
    assert((new Vector3(1,1,2)).negateThis().equalWith(new Vector3(1,-1,-2)));
  });
  it("normalize this vector test",()=>{
    assert((new Vector3(1,3,4)).normalizeThis().nearlyEqualWith(new Vector3(1,0.6,0.8)));
  });
  it("a vector to string test",()=>{
    assert((new Vector3(1,1,2)).toString()==='(1, 2)');
  });
  it("a vector to display string",()=>{
    assert((new Vector3(1,1,2)).toDisplayString()==='Vector3(1,1, 2)');
  });
  it("a vector to mathematicastring",()=>{
    assert((new Vector3(1,1,2)).toMathematicaString()==='{1, 2}');
  });
