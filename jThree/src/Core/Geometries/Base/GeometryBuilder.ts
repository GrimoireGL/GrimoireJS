...
class GeometryBuilder {
    ...
    public static addSphere(pos: number[], normal: number[], uv: number[], index: number[], divide1: number, divide2: number, center: Vector3, r: number): void {
        const vt = Vector3.add(center, new Vector3(0, r, 0));
        const vb = Vector3.add(center, new Vector3(0, -r, 0));
        pos.push(vt.X, vt.Y, vt.Z);
        normal.push(0, 1, 0);
        uv.push(0, 1);
        for (let i = 0; i < divide1 - 1; i++) {
            const angle_y = (i + 1) * Math.PI / divide1;
            const d = r * Math.sin(angle_y);
            const h = r * Math.cos(angle_y);
            for (let j = 0; j < divide2; j++) {
                const angle_x = j * 2 * Math.PI / divide2;
                const currentNormal = new Vector3(d * Math.sin(angle_x), 0, d * Math.cos(angle_x));

                const v0 = Vector3.add(new Vector3(0, h, 0), currentNormal);
