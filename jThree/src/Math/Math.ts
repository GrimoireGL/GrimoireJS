...
import IUnitConverter = require("./IUnitConverter");

class DegreeMilliSecoundUnitConverter extends jThreeObject implements IUnitConverter {
...
class jThreeMath extends jThreeObject {
    ...
    private radianResult(f: Delegates.Func0<number>): number {
        return this.converter.fromRadian(f());
    }

    private radianRequest(v: number, f: Delegates.Func1<number, number>): number {
